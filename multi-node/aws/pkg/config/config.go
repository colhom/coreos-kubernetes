...
func (c Cluster) Config() (*Config, error) {
	...
	serviceNetIP, _, err := net.ParseCIDR(config.ServiceCIDR)
	if err != nil {
		return nil, fmt.Errorf("invalid serviceCIDR: %v", err)
	}

	config.KubernetesServiceIP = incrementIP(serviceNetIP).String()
...
func (cfg Cluster) valid() error {
	...
	kubernetesServiceIP := incrementIP(serviceNetIP)
